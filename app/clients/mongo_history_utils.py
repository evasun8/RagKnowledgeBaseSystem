# Import system modules: used for reading environment variables
import os
...
def save_chat_message(
    ...
    :param image_urls: Associated image URL list (optional, defaults to None).
    ...
    document = {
        ...
        "image_urls": image_urls,  # Associated image URL list
...
def update_message_item_names(ids: List[str], item_names: List[str]) -> int:
    ...
    try:
        ...
        result = mongo_tool.chat_message.update_many(
            # Update condition: batch filtering using the specified ID list
            {
                "_id": {"$in": object_ids}
            },
